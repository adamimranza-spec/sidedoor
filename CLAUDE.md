...
3. `selectCompaniesWithContacts` (in `api/generate.js`) picks the final 3 from that pool of 10, preferring candidates a real person can actually be found at — a suggested company nobody can be found at isn't a usable suggestion. **Serper runs first, in parallel, for every candidate** — `searchPersonViaSerper` searches Google with a plain quoted `site:linkedin.com/in "{title}" "{company}"` (an earlier `intitle:`-based version over-constrained Google's ranking and suppressed genuine matches). The verification lives in code instead of the query: `resultMentionsCompany`/`resultReflectsCurrentRole` only check the returned result's page `<title>` (auto-generated from the person's current headline), never the snippet, which can quote text from anywhere on the page including past jobs — a plain "does the company appear anywhere" check was passing profiles whose *former* company or a totally different current role happened to be mentioned in their work history. Matches are also rejected if the title tag reads as a past role ("Former", "Ex-", "previously"). Only candidates Serper found nothing for fall back to a single batched Prospeo `search-person` call, bucketed back to each candidate by *normalized* company name (not exact string match — Prospeo's own `company.name` on a result can be punctuated differently than the name `search-company` gave us for the same company, e.g. "Acme Inc." vs "Acme Inc", and exact matching was silently dropping valid hits). Whichever source found a hit, `enrich-person` turns it into a verified email — but only for the 3 companies actually kept, capped at 2 contacts per company. All three Prospeo/Serper calls log their raw responses to Vercel function logs for diagnosis.

**Titles must be single, realistic LinkedIn job titles** — since `intitle:` requires an exact phrase match, a Claude-generated title like "CEO/Founder" (a natural way to write it as *category* guidance, but not how any real person titles themselves on LinkedIn) will essentially never match a real profile. Both title-generation prompts (`SYSTEM_PROMPT`, `SYSTEM_PROMPT_PERSONA`) explicitly forbid slash/"or"-joined titles now, and `splitCompoundTitles()` is a server-side safety net that splits any that slip through anyway before they reach search — don't remove either half, they cover different failure modes (prompt drift vs. one-off model noncompliance).
