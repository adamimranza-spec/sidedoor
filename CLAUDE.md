...
3. `selectCompaniesWithContacts` (in `api/generate.js`) picks the final 3 from that pool of 10, preferring candidates a real person can actually be found at — a suggested company nobody can be found at isn't a usable suggestion. **Serper runs first, in parallel, for every candidate** — `searchPersonViaSerper` does a Google search (`site:linkedin.com/in "{title}" "{company}"`) and, critically, verifies the company name actually appears in the matched result before accepting it (added after Prospeo's own `search-person` was observed returning stale/mismatched people in production — a generic company name plus no verification was producing wrong contacts). Only candidates Serper found nothing for fall back to a single batched Prospeo `search-person` call. Whichever source found a hit, `enrich-person` turns it into a verified email — but only for the 3 companies actually kept, capped at 2 contacts per company.
