...
## CONTACT & COMPANY DISCOVERY PIPELINE (Prospeo + optional Serper)

Everything that isn't Claude's writing runs through Prospeo, with Serper.dev as an optional fallback used only in discover mode's company selection step (see below).

**Job mode** — `search-person` filtered by `company.names` + `person_job_title` finds real people matching Claude's suggested decision-maker titles at the named company. `enrich-person` (given a `linkedin_url`) turns a match into a verified email. This replaced an earlier Tavily-based Google-scrape approach — `search-person` is a structured database query against Prospeo's own data, not a scrape, so match quality is higher. Unaffected by the discover-mode changes below — the user already named a specific company, so there's no "pick 3 companies" step to guard.
...
3. `selectCompaniesWithContacts` (in `api/generate.js`) picks the final 3 from that pool of 10, preferring candidates Prospeo's `search-person` can actually find someone at — a suggested company nobody can be found at isn't a usable suggestion. It enforces that at least 2 of the 3 final picks come with a real contact whenever the data allows it: a single batched `search-person` call checks all 10 candidates at once, and if fewer than 2 have hits, `searchPersonViaSerper` (Google search via Serper.dev for `site:linkedin.com/in "{title}" "{company}"`, then the found profile is run back through Prospeo's `enrich-person` for a verified email) backfills a real contact for enough of the empty candidates to reach that bar before giving up. `enrich-person` only ever runs on the contacts belonging to the 3 companies actually kept, capped at 2 per company.
...
**Cost guardrail**: discover mode is capped to 3 final companies × 2 enriched contacts (~6 `enrich-person` calls), 1 `search-company` call (pulling up to 10 candidates), 1 batched `search-person` call across all 10 candidates, up to a few `searchPersonViaSerper` calls only when Prospeo's own data falls short of the 2-with-contacts bar, and 2 Claude calls per request.

**`SERPER_API_KEY` is optional.** Without it, `selectCompaniesWithContacts` still prefers companies with Prospeo contacts, it just can't backfill when Prospeo comes up short — so the "at least 2 of 3 have contacts" guarantee is best-effort rather than close to certain. Log both `[Prospeo]` and `[Serper]` lines in Vercel function logs when diagnosing empty contact results.
